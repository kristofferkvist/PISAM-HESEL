...
class Table_cx_sampling_2D:
    ...
    def rate_contrib_T(self, T, alpha_grid, v_i_grid, v_rel_grid):
        cross_section_grid = self.cx_cross_section(v_rel_grid)
        cross_vrel = np.multiply(cross_section_grid, v_rel_grid)
        cross_vrel_v_i = np.multiply(cross_vrel, v_i_grid)
        exp_grid = np.exp(-(self.mass*np.power(v_i_grid, 2))/(2*T*1.602e-19))
        return 2/(2*np.pi)*self.mass/(T*1.602e-19)*np.multiply(exp_grid, cross_vrel_v_i)
...
class Table_light_heavy_sampling:
    def __init__(self, T_e_min, T_e_max, T_e_res, base, n_standard_deviations, v_res, sum_coefs, table_filename, mass_electron, E_thres):
        ...
        self.dvs = n_standard_deviations*np.sqrt(self.Ts*1.602e-19/mass_electron)/(v_res-1)
    ...
    def cx_cross_section(self, v_s):
        E_array = 0.5*np.power(v_s, 2)*self.mass/1.602e-19
        E_array_valid = E_array > self.E_thres
        ln_E_array = np.zeros_like(E_array)
        ln_E_array[E_array_valid] = np.log(E_array[E_array_valid])
        ln_cross_section_array = np.zeros_like(ln_E_array)
        for i in np.arange(self.sum_coefs.size):
            ln_cross_section_array[E_array_valid] = ln_cross_section_array[E_array_valid] + self.sum_coefs[i]*np.power(ln_E_array[E_array_valid], i)
        #ln_cross_section_grid[E_grid_valid] = -3.294589355000e+01 - 1.713112000000e-01*ln_E_grid[E_grid_valid]
        cross_section_array = np.exp(ln_cross_section_array)*1e-4 #Conversion from cm^2 to m^2
        cross_section_array[np.invert(E_array_valid)] = 0
        return cross_section_array

    def rate_contrib_T(self, T, v_s):
        cross_section_array = self.cx_cross_section(v_s)
...
class Table_light_heavy_sampling_mccc:
    ...
    def load_fit_params_b3(self, filename):
        fit_info = np.loadtxt(filename, usecols = (3, 4, 5, 6, 7))
        E_thres = fit_info[0]
        fit_params = fit_info[1:]
        return E_thres, fit_params

    def calc_cross_b3(self, E_s, E_thres, fit_params):
        cross_section = np.zeros(E_s.size)
        E_s_valid = E_s[E_s > E_thres]
        x = E_s_valid/E_thres
        cross_section[E_s > E_thres] = fit_params[0]*np.power(x-1, -fit_params[1])*np.exp(-fit_params[2]/np.power(x-1, fit_params[3]))
        return cross_section

    def cross_section_b3(self, v_s):
        E_s = 0.5*np.power(v_s, 2)*self.mass/1.602e-19
        E_thres, fit_params = self.load_fit_params_b3(self.file)
        return self.calc_cross_b3(E_s, E_thres, fit_params)*self.a0*self.a0

    def rate_contrib_T(self, T, v_s):
        cross_section_array = self.cross_section_b3(v_s)
        cross_v = np.multiply(cross_section_array, np.power(v_s, 3))
        exp_array = np.exp(-(self.mass*np.power(v_s, 2))/(2*T*1.602e-19))
        return np.sqrt(2/np.pi)*np.power(self.mass/(T*1.602e-19), 1.5)*np.multiply(exp_array, cross_v)
...
class Table_light_heavy_sampling_from_spline:
    def __init__(self, T_e_min, T_e_max, T_e_res, base, n_standard_deviations, v_res, input_file, table_filename, E_min, E_max):
        self.T_e_min = T_e_min
        self.T_e_max = T_e_max
        self.T_e_res = T_e_res
        self.file = input_file
    ...
    def cross_section(self, v_s):
        E_s = 0.5*np.power(v_s, 2)*self.mass/1.602e-19
        cross = np.zeros_like(E_s)
        min_mask = E_s > self.E_min
        max_mask = E_s < self.E_max
        mask = min_mask*max_mask
        E_s_valid = E_s[mask]
        cross[mask] = self.cross_spline(E_s_valid)
        return cross
