...
FastOutput::FastOutput(Options* opt) {
  ...
  if (enable_monitor || enable_timestep) {
    ...
    if (MYPE == 0)
      output_file.add(current_time, "t_array", true);

	// Add mesh options to output for it to be readable by the collect function
	bout::globals::mesh->outputVars(output_file);
  }
}
