...
class H_molecule(Species):
    ...
    def ass_ionize(self, ass_ionized):
        ...
        #Breed the new atoms
        self.diss_product.inflow(xs, ys, vxs_new, vys_new, vzs_new)
        ...
        self.domain.ion_source_particle[inds_x, inds_y] = self.domain.ion_source_particle[inds_x, inds_y] + 1
        self.domain.electron_source_momentum_x[inds_x, inds_y] = self.domain.electron_source_momentum_x[inds_x, inds_y] + self.domain.electron_mass*self.vx[0:self.max_ind][ass_ionized]
        self.domain.electron_source_momentum_y[inds_x, inds_y] = self.domain.electron_source_momentum_y[inds_x, inds_y] + self.domain.electron_mass*self.vy[0:self.max_ind][ass_ionized]
        #Remember that fragments are flying opposite to each other, hence the (-)
        self.domain.ion_source_momentum_x[inds_x, inds_y] = self.domain.ion_source_momentum_x[inds_x, inds_y] + self.domain.d_ion_mass*(-1)*vxs_new
        self.domain.ion_source_momentum_y[inds_x, inds_y] = self.domain.ion_source_momentum_y[inds_x, inds_y] + self.domain.d_ion_mass*(-1)*vys_new
        self.domain.ion_source_energy[inds_x, inds_y] = self.domain.ion_source_energy[inds_x, inds_y] + Es_fragment_cm
        self.domain.electron_source_energy[inds_x, inds_y] = self.domain.electron_source_energy[inds_x, inds_y] - (2*Es_fragment_cm + 1.61 + 15.4) + self.domain.kinetic_energy(self.vx[0:self.max_ind][ass_ionized], self.vy[0:self.max_ind][ass_ionized], self.vz[0:self.max_ind][ass_ionized], self.domain.electron_mass)
    ...
    def dissociate(self, dissociated, energy_loss, fragment_energy):
        ...
        self.domain.electron_source_energy[inds_x, inds_y] = self.domain.electron_source_energy[inds_x, inds_y] - energy_loss
