...
class H_atom(Species):
    ...
    def probs_ionization(self):
        self.probs_arr[self.ION-1, 0:self.max_ind] = self.probs_T_n(self.ion_table)
    ...
    def ion_sources(self, ionized):
        inds_x = self.plasma_inds_x[0:self.max_ind][ionized]
        inds_y = self.plasma_inds_y[0:self.max_ind][ionized]
        self.domain.electron_source_particle[inds_x, inds_y] = self.domain.electron_source_particle[inds_x, inds_y] + 1
        self.domain.electron_source_momentum_x[inds_x, inds_y] = self.domain.electron_source_momentum_x[inds_x, inds_y] + self.domain.electron_mass*self.vx[0:self.max_ind][ionized]
        self.domain.electron_source_momentum_y[inds_x, inds_y] = self.domain.electron_source_momentum_y[inds_x, inds_y] + self.domain.electron_mass*self.vy[0:self.max_ind][ionized]
        self.domain.ion_source_momentum_x[inds_x, inds_y] = self.domain.electron_source_momentum_x[inds_x, inds_y] + self.domain.d_ion_mass*self.vx[0:self.max_ind][ionized]
        self.domain.ion_source_momentum_y[inds_x, inds_y] = self.domain.electron_source_momentum_y[inds_x, inds_y] + self.domain.d_ion_mass*self.vy[0:self.max_ind][ionized]
        self.domain.electron_source_energy[inds_x, inds_y] = self.domain.electron_source_energy[inds_x, inds_y] + self.E[0:self.max_ind][ionized]*self.domain.electron_mass/self.domain.d_ion_mass - 13.6
        self.domain.ion_source_energy[inds_x, inds_y] = self.domain.ion_source_energy[inds_x, inds_y] + self.E[0:self.max_ind][ionized]

    def ionize(self, ionized):
        self.ion_sources(ionized)
        self.remove(ionized)

    #def ionize_save(self, ionized):
    #    self.ionized_save_x, self.ionized_save_y, self.n_ionized = self.save(self.ionized_save_x, self.ionized_save_y, self.n_ionized, ionized)
    #    self.remove(ionized)

    def ionize_save(self, ionized):
        self.hist_ion = self.save_to_hist_reaction(self.hist_ion, ionized)
        self.hist_ion_temp = self.save_to_hist_reaction_temperature(self.hist_ion_temp, ionized)
        self.ionize(ionized)
    ...
    def cx_sources(self, cxed, vx_new, vy_new, vz_new):
        ...
        self.domain.ion_source_momentum_x[inds_x, inds_y] = self.domain.ion_source_momentum_x[inds_x, inds_y] + self.domain.d_ion_mass*(-vx_new + vx_old)
        self.domain.ion_source_momentum_y[inds_x, inds_y] = self.domain.ion_source_momentum_y[inds_x, inds_y] + self.domain.d_ion_mass*(-vy_new + vy_old)
        self.domain.ion_source_energy[inds_x, inds_y] = self.domain.ion_source_energy[inds_x, inds_y] + self.E[0:self.max_ind][cxed] - self.domain.kinetic_energy(vx_new, vy_new, vz_new, self.domain.d_ion_mass)
    ...
    def cx(self, cxed):
        ...
        v_s_cxed = np.sqrt(np.power(self.vx[0:self.max_ind][cxed], 2) + np.power(self.vy[0:self.max_ind][cxed], 2) + np.power(self.vz[0:self.max_ind][cxed], 2))
        E_s_cxed = 0.5*self.mass/(1.6022e-19)*v_s_cxed
        E_inds = self.cx_dist_table.get_inds_E(E_s_cxed) #Same as for T
        ...
        #Velocity in primed coordinates
        vxp = v_s*np.sin(alphas)*np.cos(phis)
        vyp = v_s*np.sin(alphas)*np.sin(phis)
        vzp = v_s*np.cos(alphas)
        vz_ratio = self.vz[0:self.max_ind][cxed]/v_s_cxed
        vz_mask = vz_ratio <= -1
        vz_ratio[vz_mask] = -0.995
        vz_mask = vz_ratio >= 1
        vz_ratio[vz_mask] = 0.995
        alpha0s = np.arccos(vz_ratio)
        phi0s = np.arctan2(self.vy[0:self.max_ind][cxed], self.vx[0:self.max_ind][cxed])
        #I rotate the primed coordinate axis back onto the lab frame - hence the minuses in the line below
        vxs, vys, vzs = self.rotate_axes(vxp, vyp, vzp, -phi0s, -alpha0s)
    ...
    def excite(self, excited):
        ...
        self.domain.electron_source_energy[inds_x, inds_y] = self.domain.electron_source_energy[inds_x, inds_y] - 10.2

    def do_interaction(self, specify_interaction):
        ionized = specify_interaction == self.ION
        self.ionize(ionized)
