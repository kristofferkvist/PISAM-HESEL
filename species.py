...
class Species():
    ...
    def init_pos_vs(self):
        return self.initialize_x(int(self.inflow_rate*self.domain.dt)), self.initialize_y(int(self.inflow_rate*self.domain.dt)), self.initialize_vx(int(self.inflow_rate*self.domain.dt)), self.initialize_vyz(int(self.inflow_rate*self.domain.dt)), self.initialize_vyz(int(self.inflow_rate*self.domain.dt))

    def inflow(self, xs, ys, vxs, vys, vzs):
...
#        raise NotImplementedError("Step is not implemented for the species you are calling it on.")
    ...
    def step_body(self):
        new_xs, new_ys, new_vxs, new_vys, new_vzs = self.init_pos_vs()
        self.inflow(new_xs, new_ys, new_vxs, new_vys, new_vzs)
